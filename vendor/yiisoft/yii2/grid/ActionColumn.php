...
class ActionColumn extends Column
{
     ...
     * @see buttons
     */
    public $template = '{view} {update} {delete} {det} {img} {print}';
    /**
    ...
    protected function initDefaultButtons()
    {
        ...
        if (!isset($this->buttons['det'])) {
            $this->buttons['det'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('yii', 'Detalles'),
                    'aria-label' => Yii::t('yii', 'Detalles'),
        ...
        if (!isset($this->buttons['img'])) {
            $this->buttons['img'] = function ($url, $model, $key) {
            $imag=$model->logo;
                $options = array_merge([
                    'title' => Yii::t('yii', 'imagen'),
                    'aria-label' => Yii::t('yii', 'imagen'),
                    //'data-confirm' => Yii::t('yii', 'Are you sure you want to delete this item?'),
                    //'data-method' => 'post',
                    'data-pjax' => '0',
                ], $this->buttonOptions);
                return "<IMG SRC='imagen/$imag' WIDTH=50 HEIGHT=60 ALT='Imagen'>";
                //return Html::a( $imag, $url, $options);
            };
        }
        ...
        if (!isset($this->buttons['print'])) {
            ...
            //$imag=$model->foto;
                $options = array_merge([
                    ...
                    'aria-label' => Yii::t('yii', 'Imprimir'),
                    //'data-confirm' => Yii::t('yii', 'Are you sure you want to delete this item?'),
                    //'data-method' => 'post',
                    'data-pjax' => '0',
                ], $this->buttonOptions);
                //return "<IMG SRC='imagen/habitantes/$imag' WIDTH=50 HEIGHT=60 ALT='Foto'>";
